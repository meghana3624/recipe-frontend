...
export const Home = () => {
  ...
  useEffect(() => {
    const fetchRecipes = async () => {
      try {
        const response = await axios.get("https://recipe-island-backend.onrender.com/recipes");
    ...
    const fetchSavedRecipes = async () => {
      try {
        const response = await axios.get(
          `https://recipe-island-backend.onrender.com/recipes/savedRecipes/ids/${userID}`
        );
        console.log("Saved recipes response:", response.data); // For debugging
    ...
    fetchRecipes();
    fetchSavedRecipes();
  }, [userID]); // Include userID in the dependency array if it's dynamic

  const saveRecipe = async (recipeID) => {
    try {
      const response = await axios.put(
        "https://recipe-island-backend.onrender.com/recipes",
        { 
          recipeID, 
          userID 
        },
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://recipe-island-backend.onrender.com/recipes/search",
        { name: search },
        { headers: { authorization: cookies.access_token } }
      );
      setRecipes(response.data);
  ...
  const isRecipeSaved = (id) =>
    savedRecipes.length > 0 && savedRecipes.includes(id);
  ...
  return (
    ...
    <div className="container">
      ...
      <header className="mb-auto">
          ...
          <main className="px-3 main-content">
            <h1>Recipes for Every Craving. Explore, Experiment, Enjoy!</h1>
            <div className="content">
            <p className="lead my-3">
              <div className="cont">

              Discover a world of culinary delights on our recipe finder
              website. Explore diverse dishes, unlock your inner chef, and savor
              the flavors of home-cooked meals. Start your foodie adventure
              today!
              </div>
            </p>
